...
function ProductDetail() {
  ...
  // Fetch produk terkait berdasarkan kategori produk saat ini
  const { data: relatedProducts, error: relatedProductsError } = useSWR(
    `http://localhost:3000/products?category=${product?.category}`,
    getProduct
  );

  // Memfilter produk terkait agar tidak termasuk produk yang sedang ditampilkan
  ...
  return (
    <Layout>
      ...
      <ProdukSerupa produk={filteredRelatedProducts} />
    </Layout>
  );
}
...
// Jika menggunakan UseEffect yang kurang direkomendasikan
